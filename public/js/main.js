(function(){
    $.ajaxSetup({
        ...
        beforeSend: function(jqXHR, Obj){            
            if (Obj.url != 'management-pos/updateamounttemp' || Obj.url != 'management-project/callPersonil' || Obj.url != 'management-project/callDokumen' || !Obj.url.includes("management-project/detailProject")) {
                loading();
            }            
        },
        complete: function(){
...
function call2(url, div, tit, data) {
    loading();
    $.ajax({
        url: url,
        type:'POST',
...
function action2(url, div, tit, data) {
    loading();
    $.ajax({
        url: url,
        type:'POST',
...
function add_action(url, div, tit, data) {
    loading();
    $.ajax({
        url: url,
        type:'POST',
...
function delete_action(url, div, tit, data) {
    Swal.fire({
      title: 'Are you sure?',
         ...
         $.ajax({
            ...
            success: function(datax) {
                ...
                if (arrdata[0].trim()==='MSG') {
                    ...
                    if(arrdata[2]!=='msgbox' && arrdata[2]!==''){
                        var urlsegment = arrdata[2].split('#');
                        call(urlsegment[0], '_content_');
                        if(urlsegment.length > 1)
                        {
                            setTimeout(function(){
                                $('#'+urlsegment[1]).find('a').trigger('click');
                                clearLoading();
                            }, 800)
                        }
                    }
                    if (arrdata[1] === 'OK') {
                        ...
                        Swal.fire(
                          'Not Deleted!',
                          'Your imaginary file has not been deleted.',
                          'success'
                        )
                    }
                } else {
...
function save_post_msg_new(url, div, tit, data) {
    loading();
    $.ajax({
            type: 'POST',
            url: url,
...
function save_post_msg(formid, iddiv, divcon) {//alert('sini');
